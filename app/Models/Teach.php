...
class Teach
{
    ...
    {
        try {
            $stmt = $this->con->prepare("SELECT T.*, C.name AS class_name, C.semester, CS.name AS course_name, CS.semester AS course_semester, CS.sks, D.name AS department_name, F.name AS faculty_name FROM teaches AS T INNER JOIN classes AS C ON C.id = T.class_id INNER JOIN courses AS CS ON CS.course_code = T.course_code INNER JOIN departments AS D ON D.department_code = C.department_code INNER JOIN faculties AS F ON F.faculty_code = D.faculty_code WHERE T.lecturer_id = ? ORDER BY C.name, C.semester, T.year DESC");
            $stmt->bindParam(1, $id);
            $stmt->execute();
